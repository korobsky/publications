...
# Rationale: 
Household contact investigation is a high yield approach to active case finding for TB. In urban, densely populated areas, there is likely transmission not only in the household but also in the nearby vicinity. We aimed to investigate the potential yield of active case finding conducted within a defined geographic radius (100m) of an index case, using cases identified via household based case finding to represent "undiagnosed" TB cases.   

# Study population:   
-Index cases: All patients who lived within the study area who were diagnosed with TB and enrolled via the health facilities at any time during the study    
-Community cases: All individuals who participated in household-based screening (and who lived within the study area) who had positive (including trace) xpert results    


# Methods:   
-Plotted residential location of index patients & community cases (gps coordinates)   
-calculated a 100m buffer around each index case   
-Identified which community cases fell within any of these buffers (could be more than one)   
-Compared sociodemographic characteristics of cases that were within the buffer (i.e. would be found by this method of screening) vs outside the buffer   
-sensitivity analysis varying buffer radius   
-(to do) estimate a number needed to screen for each radius   
...
```{r CC_data, echo=FALSE}
# index cases: facility diagnosed cases from study area
cc_data<-read.csv("STOMPTBMainCasesCont_DATA_2021-09-20_1454.csv")

#keep only facility cases
   hf_cases <- cc_data %>% 
    filter (redcap_event_name=="tb_case_enrollment_arm_1" | redcap_event_name=="tb_case_enrollment_arm_3")  %>% 
    filter (participant_parish %in% 1:3) %>% 
    mutate(
    hf = case_when (
      redcap_event_name=="tb_case_enrollment_arm_1"  ~ 1,
      redcap_event_name=="tb_case_enrollment_arm_3" & com_screening_method_case==1 ~ 1,
      redcap_event_name=="tb_case_enrollment_arm_3" & com_screening_method_case>1 ~ 0))  %>% 
    filter (hf ==1) 
   
        # table(hf_cases$redcap_event_name)  
        # table(hf_cases$participant_parish)
        # table(hf_cases$hf)
...
```{r HH1_data, echo=FALSE}
hh1<-read.csv('1stSTOMPTBCommunityW_DATA_2021-09-20_1502.csv')

hh1_clean<-hh1%>%
    filter(redcap_event_name !="individual_info_arm_2") %>%
    filter(record_id>4)  %>% 
    mutate(householdID=case_when(
       redcap_event_name =="household_info_arm_1" ~ hhid , 
       redcap_event_name !="household_info_arm_1" ~ record_id ))
  
#table(hh1_clean$redcap_event_name)
#sum(is.na(hh1_clean$householdID))

#select smaller datasets
hh1_hh_info<-hh1_clean %>% 
  filter(redcap_event_name=="household_info_arm_1")  %>% 
  dplyr::select(record_id, householdID, redcap_event_name, hhid:hh_village_other)
  
hh1_ind_info<-hh1_clean %>% 
  filter(redcap_event_name !="household_info_arm_1")%>% 
  dplyr::select(householdID, record_id, redcap_event_name, recordidcalc:res_cmmt)

#only positive xperts
#0, MTB negative | 1, MTB Postive | 8, Invalid or indeterminate | 7, Result missing or unavailable | 9, Not categorized by current automated script
  #table(hh1_ind_info$xpertscreen_category)
  
  
  hh1_cases<-hh1_ind_info %>% filter (xpertscreen_category %in% 1:2)
 # table(hh1_cases$xpertscreen_category)
...
```{r HH2_data, echo=FALSE}

hh2<-read.csv('2ndSTOMPTBCommunityW_DATA_2021-09-20_1453.csv') 
#table(hh2$redcap_event_name)
hh2_clean<-hh2 %>%
    filter(redcap_event_name !="individual_info_arm_2") %>%
    filter(redcap_event_name !="costing_dtd_inside_arm_3") %>%
    filter(redcap_event_name !="costing_dtd_outsid_arm_3") %>%
    filter(redcap_event_name !="costing_vbs_arm_4")  %>% 
...
#select smaller datasets
hh2_hh_info<-hh2_clean %>% 
  filter(redcap_event_name=="household_info_arm_1")  %>% 
  dplyr::select(householdID, record_id, redcap_event_name, hhid:hh_village_other)
  
hh2_ind_info<-hh2_clean %>% 
  filter(redcap_event_name !="household_info_arm_1")%>% 
  dplyr::select(householdID,  record_id, redcap_event_name, recordidcalc:res_cmmt)

          #class(hh2_clean$hhid)
          #class(hh2_clean$record_id)
          #class(hh2_clean$record_id2)
          
          #class(hh1$hhid)
          #class(hh1$record_id)

#sum(is.na(hh2_ind_info$householdID))

  #only positive xperts
  #0, MTB negative | 1, MTB Postive | 2, MTB Trace | 8, Invalid or indeterminate | 7, Result missing or unavailable | 9, Not categorized by current automated script
  #table(hh2_ind_info$xpertscreen_category)
  hh2_cases<-hh2_ind_info %>% filter (xpertscreen_category %in% 1:2)
  #table(hh2_cases$xpertscreen_category)
...
```{r coords, echo=FALSE}
#index cases
#sum(is.na(hf_cases$gps_latitude))
#sum(is.na(hf_cases$gps_longitude))

hf_cases_gps<-hf_cases %>%
  dplyr::select(gps_latitude, gps_longitude)%>%
  filter(!is.na(gps_latitude))%>%
  filter(!is.na(gps_longitude))

#merge in gps data for each phase separately
gps1<-read.csv('GPS_Collection_for_All_Records_2August2019_results.csv')
gps1<-gps1 %>%
    distinct(householdID, .keep_all = TRUE) 
hh1_gps<-left_join(hh1_cases, gps1, by="householdID")

hh1_gps_pts<-hh1_gps %>%
  dplyr::select(householdID, screening_id, Location.Latitude, Location.Longitude)
#head(hh1_gps)


gps2<-read.csv('Community_Phase_II_GPS_Collection_results_09AUgust2021.csv')
gps2<-gps2 %>%
    distinct(householdID, .keep_all = TRUE)
hh2_gps<-left_join(hh2_cases, gps2, by="householdID")
...
all_comm_pts <- rbind(hh1_gps_pts, hh2_gps_pts)
all_comm_pts_gps<-all_comm_pts%>%
  filter(!is.na(Location.Latitude))%>%
  filter(!is.na(Location.Longitude))


#numbers included in study

#HF cases (index cases)
#count(hf_cases)
#count(hf_cases_gps)
...
# count(hh1_cases)
# count(hh2_cases)
# count(all_comm_pts_gps) # all COM cases with GPS data

all_com_gps_ids<-c(all_comm_pts_gps$screening_id)
...
#get basemap
# register_google(key = "AIzaSyBQwvvPDe8J38SS9Ig2EA-E9kDyQugDVN0")
# bw_map <- get_googlemap(center = c(32.6, .31), zoom = 15,
#   color = "bw")
# ggmap(bw_map)
